...
function ns:InitPerformancePage()
    ...
    local module = Addon and Addon.GetModule and Addon:GetModule("PerformanceSettings") or nil

    if not Addon or not module then
        local b = W.Builder(parent)
        b:Header("Performance")
        b:Desc("Performance settings and CVars for smoother gameplay.")
        b:Desc("|cffff7777Performance module is unavailable.|r")
        b:Finalize()
        return
    end
    ...
    local applyAllBtn = MakeActionButton(header, "Apply All Recommended", 180, function()
        if module and type(module.ApplyAll) == "function" then
            ...
            Addon:Print("Performance: applied " .. tostring(updated) .. " settings." .. (failed > 0 and " " .. tostring(failed) .. " failed." or ""))
        end
    ...
    local restoreBtn = MakeActionButton(header, "Restore Previous", 140, function()
        if module and type(module.RestoreAll) == "function" then
            ...
            Addon:Print("Performance: restored " .. tostring(restored) .. " settings." .. (failed > 0 and " " .. tostring(failed) .. " failed." or ""))
        end
        if ns.RefreshCurrentPage then ns.RefreshCurrentPage() end
    end)
    ...
    enableLabel:SetText("Enable Performance")
    ...
    toggle:SetChecked(Addon:IsModuleEnabled("PerformanceSettings"))
    toggle:SetScript("OnClick", function(self)
        Addon.db.modules.PerformanceSettings = Addon.db.modules.PerformanceSettings or {}
        Addon.db.modules.PerformanceSettings.enabled = self:GetChecked() and true or false
    ...
    applyAllBtn:ClearAllPoints()
    applyAllBtn:SetPoint("TOPLEFT", enableRow, "BOTTOMLEFT", 0, -10)
