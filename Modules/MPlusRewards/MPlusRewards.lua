...
Addon:RegisterModule("MPlusRewards", Module)
...
local function GetModuleMedia()
    local mediaCfg = (Addon.GetModuleMediaConfig and Addon:GetModuleMediaConfig("MPlusRewards")) or {}
    return {
        font = mediaCfg.font,
...
function Module:RefreshVisibility()
    ...
    if not Addon:IsModuleEnabled("MPlusRewards") then
        self.frame:Hide()
        return
    end
...
function Module:SetEnabled(enabled)
    Addon.db.modules.MPlusRewards.enabled = enabled and true or false
    self:Refresh()
end
