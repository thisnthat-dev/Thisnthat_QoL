    ...
    local brokerName = "TNT: Specialization"
    local menuFrame = nil
    ...
    local TalentLoadoutManager = rawget(_G, "TalentLoadoutManager")
    ...
    local function GetTalentLoadoutNameFromTalentLoadoutManager()
        ...
        if type(TalentLoadoutManager) ~= "table" then
            return nil
        end

        local activeLoadoutID = nil

        if type(TalentLoadoutManager.GetActiveLoadoutID) == "function" then
            local ok, value = pcall(TalentLoadoutManager.GetActiveLoadoutID, TalentLoadoutManager)
            if ok and (type(value) == "string" or type(value) == "number") then
                activeLoadoutID = value
            end
        end
        ...
        if activeLoadoutID ~= nil then
            ...
            if type(TalentLoadoutManager.GetLoadoutByID) == "function" then
                local okLoadoutMethod, valueLoadoutMethod = pcall(TalentLoadoutManager.GetLoadoutByID, TalentLoadoutManager, activeLoadoutID)
    ...
    local function ShowTalentSpecializationMenu()
        ...
        if easyMenu then
            ...
            ShowFallbackMenu("Talent Specialization", fallbackEntries)
        end
    end
    ...
    local function ShowLootSpecializationMenu()
        ...
        if easyMenu then
            ...
            ShowFallbackMenu("Loot Specialization", fallbackEntries)
        end
    end
