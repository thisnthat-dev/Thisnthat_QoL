...
function ns:InitDataBrokerPanelsPage()
    ...
    local sizeRefreshGeneration = 0
    local applyRefreshGeneration = 0

    local function ScheduleRightPaneRefresh()
        sizeRefreshGeneration = sizeRefreshGeneration + 1
        local generation = sizeRefreshGeneration
        C_Timer.After(0.12, function()
            if generation ~= sizeRefreshGeneration then
                return
            end
            if type(RenderRightPane) == "function" then
                RenderRightPane()
            end
        end)
    end

    local function SchedulePanelsApply()
        applyRefreshGeneration = applyRefreshGeneration + 1
        local generation = applyRefreshGeneration
        C_Timer.After(0.12, function()
            if generation ~= applyRefreshGeneration then
                return
            end
            applyPanels()
        end)
