...
function ns:InitDataBrokerBarsPage()
    ...
    local function db()
        ...
        d.showBackdrop     = d.showBackdrop ~= false
        d.showBorder       = d.showBorder ~= false
        return d
    end

    local function GetModuleDefaultColors()
        local cfg = Addon:GetModuleMediaConfig("databrokers")
        local background = cfg.textureColor or {}
        local border = cfg.borderColor or {}
        return {
            r = background.r or 0.09,
            g = background.g or 0.11,
            b = background.b or 0.14,
            a = background.a or 0.95,
        }, {
            r = border.r or 0.18,
            g = border.g or 0.24,
            b = border.b or 0.30,
    ...
    local function ensureBar(index)
        ...
        if cfg.overrideColors == nil then
            cfg.overrideColors = (type(cfg.backgroundColor) == "table") or (type(cfg.borderColor) == "table")
        else
    ...
    do
        ...
        b:Desc("Configure visibility and interaction for all bars.")
        ...
        b:Toggle("Show Bar Backdrop",
            function() return db().showBackdrop end,
            function(v) db().showBackdrop = v and true or false; applyModule() end)
        b:Toggle("Show Bar Border",
            function() return db().showBorder end,
            function(v) db().showBorder = v and true or false; applyModule() end)
        b:Finalize()
    ...
    if #d.bars == 0 then
        ...
        for index = 1, #d.bars do
            ...
            AddThreeColumnRow(b,
                function(col)
                    col:Toggle("Override Default Colors",
                        function()
                            local cur = ensureBar(index)
                            return cur and cur.overrideColors
                        end,
                        function(v)
                            local cur = ensureBar(index)
                            if not cur then return end
                            cur.overrideColors = v and true or false
                            if not cur.overrideColors then
                                cur.backgroundColor = nil
                                cur.borderColor = nil
                            end
                            applyModule()
                        end, 0)
                ...
                function(col)
                    col:Color("Bar Background Color",
                        function()
                            local cur = ensureBar(index)
                            local moduleBackgroundColor = GetModuleDefaultColors()
                            local tc = (cur and cur.overrideColors and cur.backgroundColor) or moduleBackgroundColor or {}
                            return tc.r or 0.09, tc.g or 0.11, tc.b or 0.14, tc.a or 0.95
                        end,
                        function(r, g, bv, a)
                            local cur = ensureBar(index)
                            if cur and cur.overrideColors then
                                cur.backgroundColor = { r = r, g = g, b = bv, a = a }
                                applyModule()
                            end
                        end, true, 0)
                end,
                function(col)
                    col:Color("Bar Border Color",
                        function()
                            local cur = ensureBar(index)
                            local _, moduleBorderColor = GetModuleDefaultColors()
                            local bc = (cur and cur.overrideColors and cur.borderColor) or moduleBorderColor or {}
                            return bc.r or 0.18, bc.g or 0.24, bc.b or 0.30, bc.a or 1
                        end,
                        function(r, g, bv, a)
                            local cur = ensureBar(index)
                            if cur and cur.overrideColors then
                                cur.borderColor = { r = r, g = g, b = bv, a = a }
                                applyModule()
                            end
                        end, true, 0)
                end)
