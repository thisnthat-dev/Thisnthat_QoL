...
local function PruneDeprecatedSettings(db)
    ...
    if type(db.modules) == "table" then
        if type(db.modules.databroker_bars) ~= "table" then
            db.modules.databroker_bars = {}
        end

        if db.modules.databroker_bars.enabled == nil and type(db.modules.databrokers) == "table" then
            db.modules.databroker_bars.enabled = db.modules.databrokers.enabled
        end
    end
    ...
    if type(db.moduleMedia) == "table" then
        ...
        local mplusMedia = db.moduleMedia.mplus_rewards
    ...
    if type(db.mplus_rewards) == "table" then
        db.mplus_rewards.x = nil
        db.mplus_rewards.y = nil
    end
...
function Addon:GetModuleMediaConfig(moduleName)
    ...
    if moduleName == "databrokers" or moduleName == "mplus_rewards" then
        cfg.useGlobal = false
    else
...
function Addon:GetResolvedMediaConfig(moduleName)
    ...
    if moduleName == "mplus_rewards" then
        return {
            useGlobal = false,
            font = moduleCfg.font or mplusMediaDefaults.font,
            fontSize = moduleCfg.fontSize or mplusMediaDefaults.fontSize,
            textureColor = moduleCfg.textureColor or mplusMediaDefaults.textureColor,
            borderColor = moduleCfg.borderColor or mplusMediaDefaults.borderColor,
        }
    end
...
function Addon:OpenOptions()
    if ns and ns.OptionsFrame then
        ns.OptionsFrame:Show()
        if type(ns.OpenOptionsTab) == "function" then
            ns:OpenOptionsTab("addon_settings")
        end
        return
    end
    ...
    if not category then
        self:Print("Options UI is not available yet.")
        return
    end
...
do
    ...
    if slashCmdList then
        slashCmdList.THISNTHAT_QOL = function(msg)
            ...
            if command == "m+" then
                local mplusModule = Addon:GetModule("mplus_rewards")
