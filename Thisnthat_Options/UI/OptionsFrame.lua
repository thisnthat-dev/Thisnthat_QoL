...
local function ResolveOptionsFont()
    ...
    local Addon = (type(ns.GetAddon) == "function" and ns.GetAddon()) or (ns and ns.Addon)
...
local function ApplyConfiguredAccentColor()
    ...
    if cfg.useClassAccentColor then
        local _, classTag = UnitClass("player")
        local classColor = classTag and RAID_CLASS_COLORS and RAID_CLASS_COLORS[classTag]
        if classColor then
            C.ACCENT.r = Clamp(classColor.r, 0, 1, 0.78)
            C.ACCENT.g = Clamp(classColor.g, 0, 1, 0.23)
            C.ACCENT.b = Clamp(classColor.b, 0, 1, 0.30)
            return
        end
    end
